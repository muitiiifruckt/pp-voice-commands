...
**Задачи, где применялся ИИ:** генерация каркаса проекта (структура каталогов, типовые паттерны FastAPI/SQLAlchemy/React), черновики README и данного технического описания, подсказки по интеграции VOSK и CORS.

**Промпты (пример формулировок; фактические диалоги сохраните у себя для отчёта):**

1. «Реализуй MVP: FastAPI, JWT, роли admin/operator, SQLite, загрузка аудио, VOSK, парсинг русских команд …»
2. «Добавь React-интерфейс: логин, запись с микрофона, история с фильтрами, модальное окно подтверждения с `<audio>` через fetch+blob»

**Личный вклад:** постановка требований под методичку, выбор стека, проверка сценариев MVP, настройка окружения (модель VOSK, FFmpeg), тестирование, доработка парсера под формулировки команд, правка документации под критерии сдачи.
